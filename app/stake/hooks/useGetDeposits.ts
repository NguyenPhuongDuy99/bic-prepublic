...
export function useGetDeposits(
  ...
  const depositReturn = (deposits as { amount: bigint; depositTime: bigint; tierIndex: bigint; withdrawn: boolean }[])
    .map((deposit) => {
      // @ts-ignore
      ...
      return {
        ...
        interest: currentTier ? Number(currentTier.annualInterestRate)/100 + "%" : "0%",
